...

{
    ...
    uint64		*rotintofset, *rowbytes;
    uint64		*upd_rotintofset;
    ...
    if( !TIFFGetField( tif, TIFFTAG_STRIPOFFSETS, &rotintofset ) )
    {
    ...
    upd_rotintofset = (uint64 *) _TIFFmalloc(sizeof(uint64) * length);
    for( i = 0; i < length; i++ )
        upd_rotintofset[i] = base_value + i*10;

    if( !_TIFFRewriteField( tif, TIFFTAG_STRIPOFFSETS, TIFF_LONG8, 
                            length, upd_rotintofset ) )
    {
        fprintf (stderr, "Can't rewrite STRIPOFFSETS.\n");
        goto failure;
    }

    _TIFFfree( upd_rotintofset );
    ...
    for( i = 0; i < length; i++ )
    {
        ...
        if( rotintofset[i] != expect )
        {
            fprintf( stderr, 
                     "%s:STRIPOFFSETS[%d]: Got %X:%08X instead of %X:%08X.\n",
                     filename, i, 
                     (int) (rotintofset[i] >> 32),
                     (int) (rotintofset[i]&0xFFFFFFFF),
                     (int) (expect >> 32), 
