...
ChopUpSingleUncompressedStrip(TIFF* tif)
{
	...
	netintofsets = (uint64*) _TIFFCheckMalloc(tif, nstrips32, sizeof (uint64),
				"for chopped \"StripOffsets\" array");
	if (newcounts == NULL || netintofsets == NULL) {
		/*
		...
		if (newcounts != NULL)
			_TIFFfree(newcounts);
		if (netintofsets != NULL)
			_TIFFfree(netintofsets);
		return;
	}
	/*
	...
	td->td_stripoffset = netintofsets;
